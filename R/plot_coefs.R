...
#' @description `plot_summs2` and `plot_coefs2` create regression coefficient
#'   plots with ggplot2.
...
#'   parameter(s).
#' @param alpha Transoarence tereshold
...
#'   this:
#'
#'   `list(pane_1 = c("Frost", "Illiteracy"),
#'    pane_2 = c("Population", "Area"))`
#'
#' @examples
...
#'                        "Murder Rate" = "Murder"),
#'            scale = TRUE, robust = TRUE,
#'            plot.distributions = TRUE, alpha = 0.3)
#'
#' @rdname plot_summs2
#' @export
...
plot_summs2 <- function(..., ci_level = .95, model.names = NULL, coefs = NULL,
                       omit.coefs = "(Intercept)", inner_ci_level = NULL,
                       colors = "CUD Bright", alpha = 0.7, plot.distributions = FALSE,
...
plot_coefs2 <- function(..., ci_level = .95, inner_ci_level = NULL,
  ...
  if (!is.null(groups)) {
    ...
    p <- p +
      facet_wrap(group ~ .,
                 ...
                 ncol = facet.cols,
                 scales = "free_y",
                 strip.position = facet.label.pos)
  }
